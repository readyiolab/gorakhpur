...
export default function HomeComponent({ onNavigate }: HomeProps) {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 text-white py-20 my-10">
          <div className="max-w-4xl  text-center lg:text-left animate-fadeIn">
            ...
            <div className="mt-8 sm:mt-12 flex flex-wrap gap-3 sm:gap-4 justify-center lg:justify-start">
              <div className=" backdrop-blur-md  px-4 sm:px-6 py-2 sm:py-3 rounded-full">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-lg   ${statsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div ref={servicesRef} className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8">
            {services.map((service, index) => (
              <div
                key={index}
                className={`group relative bg-white/15 p-5  rounded-2xl hover:shadow-2xl transition-all duration-500  ${servicesVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
      ...
      <section ref={whyChooseRef} className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-[#004445] via-[#2c786c] to-[#004445] text-white relative overflow-hidden">
        {/* Animated background elements */}
        <div className="absolute top-20 left-10 w-40 h-40 bg-[#f8b400]/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-20 right-10 w-40 h-40 bg-[#f8b400]/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1.5s' }}></div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {[
              ...
              <div
                key={index}
                className={`flex items-start space-x-4 bg-white/5 backdrop-blur-sm border border-white/10 p-4 sm:p-6 rounded-2xl shadow-xl  ${whyChooseVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'
                  }`}
      ...
      {/* Testimonials Section - Enhanced with ratings */}
      <section ref={testimonialsRef} className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-[#faf5e4] to-white relative overflow-hidden">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {testimonials.map((testimonial, index) => (
              <div
                key={index}
                className={`bg-white p-6 sm:p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border-2 border-transparent hover:border-[#f8b400]/20 ${testimonialsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                {/* Rating stars */}
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-[#2c786c] via-[#004445] to-[#2c786c] text-white relative overflow-hidden m-10 rounded-3xl">
        ...
        <div className="absolute inset-0 opacity-10">
          ...
          <div className="absolute bottom-0 right-1/4 w-64 h-64 bg-[#f8b400] rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
        </div>
